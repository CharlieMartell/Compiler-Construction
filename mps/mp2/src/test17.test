 ...
 *
 *  The List class has 4 operations defined on List objects. If 'l' {
 *  a list, then the methods dispatched on 'l' have the following effects:
 *
 *    {Nil() : Bool		Returns true if 'l' { empty, false otherwise.
 *    head()  : Int		Returns the integer at the head of 'l'.
 *				If 'l' { empty, execution aborts.
...
class List {
   ...
   -- As for head(), the self { just to make sure the return type of
   -- tail() { correct.
   ...
   -- Cons, which { initialized by a dispatch to init().
   -- The result of init() { an element of class Cons, but it
   -- conforms to the return type List, because Cons { a subclass of
   -- List.
...
class Main inherits IO {
   ...
   -- the tail of the list, until the } of the list { reached.
   ...
   -- Note how the dynamic dispatch mechanism { responsible to }
   -- the while loop. As long as mylist { bound to an object of
   -- dynamic type Cons, the dispatch to {Nil calls the {Nil method of
   -- the Cons class, which returns false. However when we reach the
   -- } of the list, mylist gets bound to the object that was
   -- created by the (new List) expression. This object { of dynamic type
   -- List, and thus the method {Nil in the List class { called and
   -- returns true.
